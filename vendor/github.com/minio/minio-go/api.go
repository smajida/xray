...
func (c Client) newRequest(method string, metadata requestMetadata) (req *http.Request, err error) {
	...
	isStreamingSignatureV4 := method == "PUT" && metadata.objectName != ""

	// Set sha256 sum only for non anonymous credentials.
	if !c.anonymous {
		// set sha256 sum for signature calculation only with signature version '4'.
		if !isStreamingSignatureV4 && c.signature.isV4() {
	...
	if metadata.contentLength > 0 && !isStreamingSignatureV4 {
		req.ContentLength = metadata.contentLength
	}

	// set md5Sum for content protection.
	if metadata.contentMD5Bytes != nil && !isStreamingSignatureV4 {
	...
	if !c.anonymous {
		...
		} else if c.signature.isV4() && !isStreamingSignatureV4 {
			// Add signature version '4' authorization header.
			req = s3signer.SignV4(*req, c.accessKeyID, c.secretAccessKey, location)
		} else if isStreamingSignatureV4 {
			s3signer.StreamingSignV4(req, c.accessKeyID, c.secretAccessKey, location, metadata.contentLength)
		}
	}
